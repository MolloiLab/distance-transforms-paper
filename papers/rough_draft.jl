...
md"""
This study implemented a previously proposed distance transform algorithm (Eq. 4) to decrease the time associated with previously proposed HD loss functions. The Julia programming language [CITE] was a logical choice since CPU and GPU code can be written in the same high-level language with speed similar to more common languages like C and Fortran.

```math
\text{Insert DT Algorithm Here} \\
```

```math
\text{Make sure the notation matches the notation below in Eq 5}
```


This study adapted the Felzenszwalb distance transform algorithm [CITE]. The Felzenszwalb algorithm is easily parallelizable, which makes it simple to provide a multi-threaded CPU and GPU-accelerated form. Previous HD loss functions utilized a readily available distance transform algorithm from Scipy [CITE].
...
md"""
Two and three-dimensional arrays from size {insert} up to size {insert} were input into the distance transforms. The distance transform utilized in this paper (``\text{DT}_{FelzenszwalbGPU}``) was shown to be the fastest implementation for two and three-dimensional arrays within the size range of common medical images, like computed tomography scans [CITE]. For the largest-sized two-dimensional array, ``\text{DT}_{FelzenszwalbGPU}`` was ``[insert] \times`` faster than ``\text{DT}_{Scipy}``. For the largest-sized three-dimensional array, ``\text{DT}_{FelzenszwalbGPU}`` was ``[insert] \times`` faster than ``\text{DT}_{Scipy}``. Figure 1 shows the two, and three-dimensional timings of the various distance transform operations.
"""
...
md"""
Two and three-dimensional arrays from size [INSERT] up to size [INSERT] were input into the distance transforms. The HD loss function, implemented in this paper (``\text{HD}_{FelzenszwalbGPU}``), was faster than any previous implementation. Specifically, ``\text{HD}_{FelzenszwalbGPU}`` was ``[INSERT]`` faster than ``\text{HD}_{SciPy}`` on the largest two-dimensional arrays and ``[INSERT]`` faster on the largest three-dimensional arrays. The input arrays in the training loop were of size ``96 \times 96 \times 96`` (Fig. 2B) and for this size, the ``\text{HD}_{FelzenszwalbGPU}`` was ``[INSERT]`` faster than ``\text{HD}_{SciPy}``. Figure 2 shows the two, and three-dimensional timings of the various loss functions.
"""
...
md"""
A simplified training loop was run for 250 epochs, and the average epoch time was examined for each loss function. Based on previous reports [CITE], combining the HD loss function with the Dice loss function is recommended to avoid instability issues. Therefore, three different loss functions were analyzed: 1) pure Dice loss function (``\text{Loss}_{DSC}``), which serves as the baseline, 2) hybrid HD loss and Dice loss function (``\text{Loss}_{Scipy}``), which uses the previously reported HD loss function [CITE], and 3) our new hybrid HD loss and Dice loss function (``\text{Loss}_{FelzenszwalbGPU}``), which runs entirely on the GPU.

The average epoch time for the previously proposed HD loss function ``\text{Loss}_{Scipy}`` was [INSERT]% slower than the baseline ``\text{Loss}_{DSC}`` (Fig. 4). The average step time for our new HD loss function ``\text{Loss}_{FelzenszwalbGPU}`` was [INSERT]% slower than the baseline per epoch (Fig. 4). 
