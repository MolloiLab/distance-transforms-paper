...
md"""
#### What is image segmentation (1-3)
...
#### Medical image segmentation (3-5)
...
#### How to evaluate segmentations (3-5)
...
```math
\begin{aligned}
hd(X, Y) = max_{x \varepsilon X} \ [min_{y \varepsilon Y} (||x-y||_{2})]
\end{aligned}
\tag{1}
```
...
```math
\begin{aligned}
hd(Y, X) = max_{y \varepsilon Y} \ [min_{x \varepsilon X} (||x-y||_{2})]
\end{aligned}
\tag{2}
```
...
#### HD loss research developments
Recently, segmentation approaches, specifically deep learning-based methods, have focused on directly minimizing the HD during the training process via a novel loss function, termed the Hausdorff (HD) loss [CITE]. The most accurate form of HD loss utilizes a distance transform operation [CITE] to estimate the HD.

Although the HD loss function improves the segmentation error, especially along the boundaries, this loss function is unstable, especially in the early part of training [CITE]. A weighted combination of loss functions can solve this issue, like the Dice loss and HD loss [CITE]. 
...
#### Our contributions
This study presents a new distance transform operation for the HD loss function. This HD loss function improves the speed of operation compared to the previous implementations and provides a GPU-optimized algorithm for deep-learning-based medical image segmentation.
"""
...
# ╔═╡ ea3b0bcb-1a9d-4c91-ae9b-01212aaaaf8c
md"""
### 2.1.1 - Formula
"""
...
```math
\begin{aligned}
Loss_{HD}(q, p) = \frac{1}{|\Omega|} \sum_{\Omega}((p - q)^{2} \circ (d_{p}^{\alpha} + d_{q}^{\alpha}))
\end{aligned}
\tag{4}
```

Where ``p`` is the binary ground-truth segmentation, ``q`` is the binary prediction, ``d_{p}`` is the distance transform of ``p``, and ``d_{q}`` is the distance transform of ``q``. The parameter ``\alpha`` determines how strongly the larger errors are penalized. Previous reports show that values of ``\alpha`` between 1 and 3 led to good results. This loss function is a good estimator of HD [CITE].
"""
...
# ╔═╡ 80954655-56a4-41a3-b885-8786761a9b0c
md"""
### 2.2.1 - Formula
"""
